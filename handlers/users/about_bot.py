...
async def cq_about(call: CallbackQuery):
    ...
    btn5 = types.InlineKeyboardButton(text='Код GitHub', url='https://github.com/vR4eslav/editorbot')
    keyboard.add(btn5)
    btn6 = types.InlineKeyboardButton(text='🔙 НАЗАД В МЕНЮ 🔙', callback_data='back_to_menu')
    keyboard.add(btn6)
    await call.message.edit_text(text=f"Отправь текст, который хочешь проверить, и получи результат! Объем одной "
                                      ...
                                      f"Сгенерировать надежный пароль\n\n"
